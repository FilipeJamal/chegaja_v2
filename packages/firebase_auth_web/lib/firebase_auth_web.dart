...
class FirebaseAuthWeb extends FirebaseAuthPlatform {
  ...
  static void registerWith(Registrar registrar) {
    ...
    FirebaseCoreWeb.registerService(
      ...
      ensurePluginInitialized: (firebaseApp) async {
        ...
        // If running on a local host, reuse the emulator origin across reloads.
        if (_isLocalAuthHost()) {
  ...
  Future<void> useAuthEmulator(String host, int port) async {
    try {
      ...
      // Save to session storage so that the emulator is reused on refreshes
      // while running against a local host.
      if (_isLocalAuthHost()) {
