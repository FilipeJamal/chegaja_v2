...
std::queue<std::function<void()>> FirebaseAuthPlugin::callbacks{};
std::mutex FirebaseAuthPlugin::callbacks_mutex{};
std::function<HWND()> FirebaseAuthPlugin::get_root_window{};
...
FirebaseAuthPlugin::FirebaseAuthPlugin(
    std::function<int(std::function<std::optional<LRESULT>(
        HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)>)>
        registrator,
    std::function<void(int)> unregistrator,
    std::function<HWND()> window_provider)
    : win_proc_delegate_registrator_(std::move(registrator)),
      win_proc_delegate_unregistrator_(std::move(unregistrator)) {
  firebase::SetLogLevel(firebase::kLogLevelVerbose);
  get_root_window = std::move(window_provider);
  if (win_proc_delegate_registrator_) {
...
FirebaseAuthPlugin::~FirebaseAuthPlugin() {
  if (win_proc_delegate_unregistrator_ && window_proc_id_ != -1) {
    win_proc_delegate_unregistrator_(window_proc_id_);
  }
}

void FirebaseAuthPlugin::RunOnMainThread(std::function<void()> callback) {
  HWND hwnd = get_root_window ? get_root_window() : nullptr;
  if (!hwnd) {
    callback();
    return;
  }
  ...
  PostMessage(hwnd, WM_FIREBASE_AUTH_RUN_CALLBACK, 0, 0);
}

std::optional<LRESULT> FirebaseAuthPlugin::HandleWindowProc(HWND hwnd,
                                                            UINT message,
                                                            WPARAM wparam,
                                                            LPARAM lparam) {
  if (message == WM_FIREBASE_AUTH_RUN_CALLBACK) {
 ...
 public:
  ...
  void OnIdTokenChanged(Auth* auth) override {
    ...
    if (user.is_valid()) {
      ...
      EncodableValue payload = EncodableValue(
          EncodableMap{{EncodableValue("user"), userDetailsList}});
      FirebaseAuthPlugin::RunOnMainThread([sink, payload]() {
        sink->Success(payload);
      });
    } else {
      EncodableValue payload = EncodableValue(EncodableMap{
