...
#define WM_FIREBASE_AUTH_RUN_CALLBACK (WM_USER + 102)
...
class FirebaseAuthPlugin : public flutter::Plugin,
 ...
 private:
  ...
  static std::function<HWND()> get_root_window;
  ...
  std::function<int(std::function<std::optional<LRESULT>(
      HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)>)>
      win_proc_delegate_registrator_;
  std::function<void(int)> win_proc_delegate_unregistrator_;
