          ...
          children: [
            Expanded(
              child: OutlinedButton(
                onPressed: () async {
                  try {
                    await PedidoService.instance.recusarConvitePrestador(
                      pedido: pedido,
                      prestadorId: uid,
                    );
                    if (context.mounted) {
...
class _AcaoEnviarOrcamento extends StatelessWidget {
  ...
  const _AcaoEnviarOrcamento({required this.pedido});

  @override
  Widget build(BuildContext context) {
    // Só o prestador que aceitou deve enviar orçamento
    final user = AuthService.currentUser;
    final uid = user?.uid;

    // Se por algum motivo não tiver user, não mostra ação
    if (uid == null) return const SizedBox.shrink();

    // Se não for o prestador atribuído, também não mostra
    if (pedido.prestadorId != null && pedido.prestadorId != uid) {
      return const SizedBox.shrink();
    }
        ...
        onPressed: () async {
          ...
          if (min == null || max == null || min <= 0 || max <= 0 || min > max) {
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Preenche min/max corretamente (min <= max).'),
                ),
              );
            }
            return;
