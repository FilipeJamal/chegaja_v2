...
class AvaliacaoRepo {
  ...
  Future<List<Avaliacao>> getAvaliacoesPrestador(String prestadorId,
      {int limit = 10}) async {
  ...
  Future<List<Avaliacao>> _enrichWithUserData(List<Avaliacao> list) async {
    ...
    final userDocs = await Future.wait(
      userIds.map((uid) => _db.collection('public_profiles').doc(uid).get()),
    );
    ...
    final userMap = {
      for (var doc in userDocs)
        doc.id: {
          'displayName':
              (doc.data()?['displayName'] ?? 'Cliente ChegaJá') as String,
