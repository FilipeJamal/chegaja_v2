...
// Gera um catálogo amplo de serviços com IDs estáveis e keywords.
...
List<Map<String, dynamic>> buildCatalogoCompleto({int? limit}) {
  ...
  void addItem(Map<String, dynamic> item) {
    ...
    final name = _humanizeDisplayText((item['name'] ?? '').toString().trim());
...
const _actionsTarefas = [
  _Action('Serviço de', ['servico']),
  _Action('Assistência em', ['assistencia']),
  _Action('Fazer', ['fazer', 'executar']),
  _Action('Organização de', ['organizacao']),
];

const _actionsDesporto = [
  _Action('Aulas de', ['aulas', 'treino']),
  _Action('Treino de', ['treino', 'coach']),
  _Action('Personal Trainer de', ['pt', 'personal']),
  _Action('Instrutor de', ['instrutor']),
];

const _actionsSaude = [
  _Action('Consulta de', ['consulta']),
  _Action('Sessão de', ['sessao', 'terapia']),
  _Action('Tratamento de', ['tratamento']),
  _Action('Acompanhamento de', ['acompanhamento']),
];

// --- NOVAS LISTAS DE OBJETOS EXPANDIDAS ---

final _instrumentosMusicais = [
  'guitarra',
  'piano',
  'bateria',
  'violino',
...
final _tarefasPessoais = [
  ...
  'pagamento de contas',
  'organizacao de closet',
  'organizacao de garagem',
  'mudanca de layout',
  'montagem de arvore de natal',
];

final _saudeMentalFisica = [
  'psicologia',
  'nutricao',
  'fisioterapia',
  'osteopatia',
...
final _groups = <_Group>[
  _Group(
    ...
    includeBase: true,
    baseId: _stableId(
        'Casa e Obras', 'Reparos Domesticos', 'Servicos de reparacao'),
  ...
  // --- NOVOS GRUPOS ---
  _Group(
    macro: 'Lifestyle e Tarefas',
    sub: 'Assistente Pessoal',
    base: 'Assistente pessoal',
    mode: 'AGENDADO',
    iconKey: 'personal_shopper',
    actions: _actionsTarefas,
    objects: _expandObjects(_tarefasPessoais, const []),
    synonyms: ['assistente virtual', 'concierge', 'ajudante'],
  ),
  _Group(
    macro: 'Desporto e Fitness',
    sub: 'Artes Marciais e Lutas',
    base: 'Artes marciais',
    mode: 'AGENDADO',
    iconKey: 'artes_marciais',
    actions: _actionsDesporto,
    objects: _expandObjects(
        _artesMarciais, ['para criancas', 'para adultos', 'defesa pessoal']),
  ),
  _Group(
    macro: 'Desporto e Fitness',
    sub: 'Fitness e Danca',
    base: 'Fitness e danca',
    mode: 'AGENDADO',
    iconKey: 'fitness',
    actions: _actionsDesporto,
    objects: _expandObjects(_fitness, ['online', 'presencial', 'outdoor']),
  ),
  _Group(
    macro: 'Educacao',
    sub: 'Musica e Artes',
    base: 'Aulas de musica',
    mode: 'AGENDADO',
    iconKey: 'musica',
    actions: _actionsEducacao,
    objects: _expandObjects(_instrumentosMusicais, ['iniciante', 'avancado']),
  ),
  _Group(
    macro: 'Educacao',
    sub: 'Linguas',
    base: 'Aulas de linguas',
    mode: 'AGENDADO',
    iconKey: 'linguas',
    actions: _actionsEducacao,
    objects:
        _expandObjects(_linguas, ['conversacao', 'gramatica', 'intensivo']),
  ),
  _Group(
    macro: 'Saude e Bem-estar',
    sub: 'Saude Especializada',
    base: 'Saude ao domicilio',
    mode: 'AGENDADO',
    iconKey: 'saude',
    actions: _actionsSaude,
    objects: _expandObjects(_saudeMentalFisica, ['ao domicilio', 'online']),
  ),
];
...
List<Map<String, dynamic>> _baseServicosFixos() {
  return [
    _fixed(
        'canalizador', 'Canalizador', 'IMEDIATO', ['agua', 'fuga', 'torneira']),
    _fixed(
        'eletricista', 'Eletricista', 'IMEDIATO', ['luz', 'tomada', 'quadro']),
    _fixed('pintor', 'Pintor', 'POR_PROPOSTA', ['pintura', 'paredes', 'tinta']),
    _fixed(
        'pedreiro', 'Pedreiro', 'POR_PROPOSTA', ['obra', 'paredes', 'reboco']),
    _fixed(
        'serralheiro', 'Serralheiro', 'IMEDIATO', ['portao', 'grade', 'ferro']),
    _fixed('carpinteiro', 'Carpinteiro', 'POR_PROPOSTA', ['madeira', 'moveis']),
    _fixed('montagem_moveis', 'Montagem de moveis', 'AGENDADO',
        ['montagem', 'armario']),
    _fixed('barbeiro', 'Barbeiro', 'AGENDADO', ['barba', 'corte', 'degrade']),
    _fixed('cabeleireiro', 'Cabeleireiro', 'AGENDADO',
        ['corte', 'escova', 'coloracao']),
    _fixed(
        'babysitter', 'Babysitter', 'AGENDADO', ['ama', 'cuidador infantil']),
    _fixed('cuidador_idosos', 'Cuidador de idosos', 'AGENDADO',
        ['acompanhante', 'apoio domiciliario']),
    _fixed('dogwalker', 'Dog walker', 'IMEDIATO', ['passeio', 'caes']),
    _fixed('pet_sitter', 'Pet sitter', 'AGENDADO', ['cuidador de animais']),
    _fixed('confeitaria', 'Confeitaria', 'POR_PROPOSTA',
        ['bolo', 'cake', 'doces']),
    _fixed('cake_designer', 'Cake designer', 'POR_PROPOSTA',
        ['bolo personalizado', 'fondant']),
    _fixed('bolos_personalizados', 'Bolos personalizados', 'POR_PROPOSTA',
        ['bolo', 'personalizado']),
    _fixed('retratista_lapis', 'Retratista a lapis', 'POR_PROPOSTA',
        ['retrato', 'lapis', 'grafite']),
    _fixed('caricaturista', 'Caricaturista', 'POR_PROPOSTA',
        ['caricatura', 'retrato']),
...
List<String> _normalizeKeywords(Set<String> raw) {
  final list =
      raw.map((k) => k.trim()).where((k) => k.isNotEmpty).toSet().toList();
  list.sort();
  if (list.length > 60) {
...
String _humanizeDisplayText(String input) {
  if (input.isEmpty) return input;

  const replacements = <MapEntry<String, String>>[
    MapEntry('servicos', 'serviços'),
    MapEntry('reparacao', 'reparação'),
    MapEntry('reparacoes', 'reparações'),
    MapEntry('diagnostico', 'diagnóstico'),
    MapEntry('avaliacao', 'avaliação'),
    MapEntry('configuracao', 'configuração'),
    MapEntry('construcao', 'construção'),
    MapEntry('demolicao', 'demolição'),
    MapEntry('reforco', 'reforço'),
    MapEntry('higienizacao', 'higienização'),
    MapEntry('remocao', 'remoção'),
    MapEntry('distribuicao', 'distribuição'),
    MapEntry('criacao', 'criação'),
    MapEntry('implementacao', 'implementação'),
    MapEntry('otimizacao', 'otimização'),
    MapEntry('integracao', 'integração'),
    MapEntry('correcao', 'correção'),
    MapEntry('producao', 'produção'),
    MapEntry('edicao', 'edição'),
    MapEntry('sessao', 'sessão'),
    MapEntry('ilustracao', 'ilustração'),
    MapEntry('coloracao', 'coloração'),
    MapEntry('decoracao', 'decoração'),
    MapEntry('organizacao', 'organização'),
    MapEntry('preparacao', 'preparação'),
    MapEntry('mudanca', 'mudança'),
    MapEntry('mudancas', 'mudanças'),
    MapEntry('logistica', 'logística'),
    MapEntry('mecanica', 'mecânica'),
    MapEntry('tecnica', 'técnica'),
    MapEntry('tecnicas', 'técnicas'),
    MapEntry('eletrodomesticos', 'eletrodomésticos'),
    MapEntry('domesticos', 'domésticos'),
    MapEntry('eletrico', 'elétrico'),
    MapEntry('eletrica', 'elétrica'),
    MapEntry('iluminacao', 'iluminação'),
    MapEntry('corrimoes', 'corrimões'),
    MapEntry('portoes', 'portões'),
    MapEntry('portao', 'portão'),
    MapEntry('armarios', 'armários'),
    MapEntry('armario', 'armário'),
    MapEntry('moveis', 'móveis'),
    MapEntry('catalogo', 'catálogo'),
    MapEntry('aplicacao', 'aplicação'),
    MapEntry('automatizacao', 'automatização'),
    MapEntry('anuncios', 'anúncios'),
    MapEntry('video', 'vídeo'),
    MapEntry('videos', 'vídeos'),
    MapEntry('grafico', 'gráfico'),
    MapEntry('artistico', 'artístico'),
    MapEntry('estetica', 'estética'),
    MapEntry('explicacoes', 'explicações'),
    MapEntry('formacao', 'formação'),
    MapEntry('educacao', 'educação'),
    MapEntry('saude', 'saúde'),
    MapEntry('domicilio', 'domicílio'),
    MapEntry('domiciliario', 'domiciliário'),
    MapEntry('domiciliaria', 'domiciliária'),
    MapEntry('musica', 'música'),
    MapEntry('linguas', 'línguas'),
    MapEntry('danca', 'dança'),
    MapEntry('avancado', 'avançado'),
    MapEntry('conversacao', 'conversação'),
    MapEntry('gramatica', 'gramática'),
    MapEntry('nutricao', 'nutrição'),
    MapEntry('fisica', 'física'),
    MapEntry('linfatica', 'linfática'),
    MapEntry('caes', 'cães'),
    MapEntry('lapis', 'lápis'),
    MapEntry('frances', 'francês'),
    MapEntry('alemao', 'alemão'),
    MapEntry('japones', 'japonês'),
    MapEntry('portugues', 'português'),
    MapEntry('ingles', 'inglês'),
    MapEntry('arabe', 'árabe'),
    MapEntry('criancas', 'crianças'),
    MapEntry('aniversarios', 'aniversários'),
  ];

  var result = input;
  for (final replacement in replacements) {
    result = result.replaceAllMapped(
      RegExp('\\b${replacement.key}\\b', caseSensitive: false),
      (match) => _applyCasePattern(match.group(0)!, replacement.value),
    );
  }
  return result;
}

String _applyCasePattern(String source, String replacement) {
  if (source.isEmpty) return replacement;
  if (source == source.toUpperCase()) {
    return replacement.toUpperCase();
  }

  final first = source.substring(0, 1);
  final rest = source.substring(1);
  final isTitleCase =
      first == first.toUpperCase() && rest == rest.toLowerCase();
  if (isTitleCase) {
