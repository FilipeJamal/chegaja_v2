...
Future<void> _initFirebase() async {
  ...
  _clientApp = await _getOrCreateApp('m16_cliente');
  _providerApp = await _getOrCreateApp('m16_prestador');
...
Future<String> _createPedidoAsClient({
  ...
  final ref = await db.collection('pedidos').add({
    ...
    'titulo': title,
    'descricao': 'Pedido criado pelo teste M1.6',
    'modo': 'IMEDIATO',
    'tipoPreco': tipoPreco,
    ...
    'latitude': null,
    'longitude': null,
    'enderecoTexto': 'Rua de teste M1.6',
  });
  return ref.id;
}
...
Servico _testService(String suffix) {
  ...
  return Servico(
    id: 'svc_m16_$safeSuffix',
    name: 'Canalizador M1.6 $safeSuffix',
    mode: 'IMEDIATO',
...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final title =
          'M16 Cliente UI normal ${DateTime.now().microsecondsSinceEpoch}';
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final pedidoId = await _createPedidoAsClient(
        ...
        service: service,
        title:
            'M16 Prestador UI normal ${DateTime.now().microsecondsSinceEpoch}',
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final pedidoId = await _createPedidoWithClienteUi(
        ...
        service: service,
        title:
            'M16 Cliente UI orcamento ${DateTime.now().microsecondsSinceEpoch}',
        modo: 'ORCAMENTO',
        clienteId: client.uid,
      );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final pedidoId = await _createPedidoAsClient(
        ...
        service: service,
        title:
            'M16 Prestador UI orcamento ${DateTime.now().microsecondsSinceEpoch}',
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await ChatService(firestore: _defaultDb, auth: _defaultAuth).sendMessage(
        pedidoId: pedidoId,
        text: 'Mensagem cliente Windows M1.6',
        senderRole: 'cliente',
      );
      await ChatService(firestore: _providerDb, auth: _providerAuth)
          .sendMessage(
        pedidoId: pedidoId,
        text: 'Resposta prestador simulado M1.6',
      ...
      final messages = await _eventually(
        ...
        'mensagens do chat M1.6',
      );

      final texts =
          messages.map((doc) => (doc.data()['text'] ?? '').toString()).toSet();
      expect(texts, contains('Mensagem cliente Windows M1.6'));
      expect(texts, contains('Resposta prestador simulado M1.6'));
    },
    skip: !_shouldRun,
  );
}
